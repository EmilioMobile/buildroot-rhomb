...
function flash_kernel_rootfs () {
        ...
        mkfs.ext4 /dev/$1p1
        mkdir -p /mnt/tmp
        mount /dev/$1p1 /mnt/tmp
        tar -xf rootfs.tar -C /mnt/tmp/
        cp zImage.exynos4412-rhomb-expansion /mnt/tmp/zImage
	fw_setenv bootcmd 'ext4load mmc 1:1 0x40008000 zImage;bootm 0x40008000'
	fw_setenv bootargs 'console=tty1 console=ttySAC1,115200 mem=1023M root=/dev/mmcblk1p1 rootwait rw'
        sync
        umount /dev/$1p1
}
